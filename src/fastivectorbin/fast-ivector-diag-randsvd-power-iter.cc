// fastivectorbin/fast-ivector-diag-randsvd-power-iter.cc
...
int main(int argc, char *argv[]) {
  ...
  try {
      ...
      "Usage:  fast-ivector-diag-randsvd-power-iter [options] <NS-stats-rxfilename> <F-stats-rspecifier> <mat-rxfilename> <mat-wxfilename>\n"
      "e.g.: fast-ivector-diag-randsvd-power-iter  norm.params ark:stats_F.1 Y.powiter0.mat Y.powiter1.1.mat \n";
    ...
    while(! stats_reader.Done()) {
      ...
      // Y_temp = F' * Y
      Y_temp.AddMatMat(1.0,F_submat,kNoTrans,Y,kNoTrans,1.0);
      // Y_out = Y_out + F * Y_temp
      Y_out.AddMatMat(1.0,F_submat,kTrans,Y_temp,kNoTrans,1.0);
      count = count + j;
    }  
