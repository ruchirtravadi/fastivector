// fastivectorbin/fast-ivector-full-randsvd-est.cc
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    t = time.Elapsed();
    Matrix<BaseFloat> S_Inv_T(m,k,kSetZero);
    for(int32 c = 0; c < num_gauss; c++) {
      SubMatrix<BaseFloat> Uc(U,c*feat_dim,feat_dim,0,k);
      SubMatrix<BaseFloat> S_Inv_Tc(S_Inv_T,c*feat_dim,feat_dim,0,k);
      S_Inv_Tc.AddTpMat(1.0,invsqrt_S[c],kNoTrans,Uc,kNoTrans,1.0);
      // // Final Recipe edit
      // S_Inv_Tc.Scale(pow(P(c),-0.5));
    }
