...
if [ $# != 3 ]; then
  ...
  echo " e.g.: $0 --nj 16 data/train 512 exp/fastivec_full"
...
# Train UBM
if [ $stage -le 0 ]; then
  # Initialize a diagonal UBM
  steps/fastivec_full/train_diag_ubm.sh \
    --nj $nj \
    --cmd $cmd \
  ...
  # Train full UBM
  steps/fastivec_full/train_full_ubm.sh \
    --nj $nj \
    --cmd $cmd \
    --num-iters $num_iters_ubm \
    --num-gselect $num_gselect_ubm \
    ...
    --remove-low-count-gaussians $remove_low_count_gaussians_ubm \
    ${vad_scp:+ --vad-scp "$vad_scp"} \
    $data $outdir/ubm $outdir/ubm
fi

# Accumulate stats
if [ $stage -le 1 ]; then
  steps/fastivec_full/acc_stats.sh \
    --ivec-dim $ivec_dim \
    --p $p \
    --nj $nj \
    ...
    ${vad_scp:+ --vad-scp "$vad_scp"} \
    $outdir/ubm/final.ubm $data $outdir/stats
fi
   
# Estimate ivector model from stats
if [ $stage -le 2 ]; then
  steps/fastivec_full/est_from_stats.sh \
