...
# sliding-window CMVN. We initialize the GMM using gmm-global-init-from-feats,
...
if [ -z "$vad_scp" ]; then
  ...
  all_feats="ark,s,cs:add-deltas $delta_opts scp:$data/feats.scp ark:- | apply-cmvn-sliding --norm-vars=false --center=true --cmn-window=300 ark:- ark:- | select-voiced-frames ark:- scp:$vad_scp ark:- |"
  feats="ark,s,cs:add-deltas $delta_opts scp:$sdata/JOB/feats.scp ark:- | apply-cmvn-sliding --norm-vars=false --center=true --cmn-window=300 ark:- ark:- | select-voiced-frames ark:- scp:$vad_scp ark:- | subsample-feats --n=$subsample ark:- ark:- |"
fi
