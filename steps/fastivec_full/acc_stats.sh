...
if [ $# != 3 ]; then
  ...
  echo " e.g.: $0 exp/ivec_mdl/ubm/final.ubm data/train exp/fast_ivec_mdl/stats"
...
ubm_path=`dirname $ubm_mdl`

# Do Gaussian selection and posterior extraction
if [ $stage -le -2 ]; then
  fgmm-global-to-gmm $ubm_mdl $ubm_path/full_to_diag.dubm
fi
...
if [ $stage -le -1 ]; then
  ...
  $cmd JOB=1:$nj $stats_dir/log/gselect.JOB.log \
    gmm-gselect --n=$num_gselect $ubm_path/full_to_diag.dubm "$feats" ark:- \| \
    fgmm-global-gselect-to-post --min-post=$min_post $ubm_mdl "$feats" \
...
post_dim=$(fgmm-global-info $ubm_mdl | grep number | cut -f 4 -d " ")
$cmd JOB=1:$nj $stats_dir/log/acc_stats.JOB.log \
  fast-ivector-full-acc-stats \
    --p=$p \
    --ivec-dim=$ivec_dim \
