...
if [ $stage -le 1 ]; then
  ...
  fast-ivector-diag-combine-stats $stats_dir/stats_NFS.* $stats_dir/stats_NFS.global
...
if [ $stage -le 2 ]; then
  ...
  $cmd JOB=1:$nj $out_dir/log/est_y.JOB.log \
    fast-ivector-diag-randsvd-get-y --ivec-dim=$ivec_dim $stats_dir/stats_NFS.global \
...
if [ $stage -le 3 ]; then
  ...
  for pow_it in `seq 1 ${q}`; do
    ...
    $cmd JOB=1:$nj $out_dir/log/pow_iter${q}.JOB.log \
      fast-ivector-diag-randsvd-power-iter --ivec-dim=$ivec_dim \
...
if [ $stage -le 4 ]; then
  ...
  $cmd JOB=1:$nj $out_dir/log/est_b.JOB.log \
    fast-ivector-diag-randsvd-get-b --ivec-dim=$ivec_dim $stats_dir/stats_NFS.global \
      ark:$stats_dir/stats_N.JOB ark:$stats_dir/stats_F.JOB \
...
if [ $stage -le 5 ]; then
  fast-ivector-diag-randsvd-est --binary=$binary --ivec-dim=$ivec_dim $out_dir/B.* \
