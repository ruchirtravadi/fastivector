...
if [ $stage -le 2 ]; then
  ...
  $cmd JOB=1:$nj $out_dir/log/est_y.JOB.log \
    fast-ivector-diag-randsvd-get-y $stats_dir/NS.stats ark:$stats_dir/stats_F.JOB $out_dir/Y.poweriter0.JOB.mat 
...
if [ $stage -le 3 ]; then
  ...
  for pow_it in `seq 1 ${q}`; do
    ...
    $cmd JOB=1:$nj $out_dir/log/pow_iter${q}.JOB.log \
      fast-ivector-diag-randsvd-power-iter $stats_dir/NS.stats ark:$stats_dir/stats_F.JOB $out_dir/Y.poweriter$((q-1)).mat $out_dir/Y.poweriter${q}.JOB.mat
...
if [ $stage -le 3 ]; then
  start=$SECONDS
  fast-ivector-diag-randsvd-get-q $out_dir/Y.poweriter${q}.mat $out_dir/Q.mat
...
if [ $stage -le 4 ]; then
  ...
  $cmd JOB=1:$nj $out_dir/log/est_b.JOB.log \
    fast-ivector-diag-randsvd-get-b $stats_dir/NS.stats ark:$stats_dir/stats_F.JOB $out_dir/Q.mat $out_dir/B.JOB.mat
  echo "Obtained the B matrices in  $(( SECONDS - start )) sec"
fi

# Get the final model by SVD
if [ $stage -le 5 ]; then
  fast-ivector-diag-randsvd-est --binary=$binary $out_dir/B.* $out_dir/Q.mat $stats_dir/NS.stats $ubm_mdl $out_dir/ivec.mdl
fi
